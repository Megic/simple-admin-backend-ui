...
<script lang="ts">
  ...
  import { getStudentList, deleteStudent } from '/@/api/iu/student';

  export default defineComponent({
    name: 'StudentManagement',
    components: { BasicTable, StudentDrawer, TableAction, Button, DeleteOutlined },
    setup() {
      const { t } = useI18n();
      ...
      const [registerTable, { reload }] = useTable({
        title: t('iu.student.studentList'),
        api: getStudentList,
        columns,
        formConfig: {
          labelWidth: 120,
      ...
      async function handleDelete(record: Recordable) {
        const result = await deleteStudent({ ids: [record.id] });
        if (result.code === 0) {
          await reload();
        }
      }
      ...
      async function handleBatchDelete() {
        Modal.confirm({
          ...
          async onOk() {
            const result = await deleteStudent({ ids: selectedIds.value as number[] });
            if (result.code === 0) {
