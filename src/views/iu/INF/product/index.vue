...
<script lang="ts">
  ...
  import { getProductList, deleteProduct } from '/@/api/iu/product';

  export default defineComponent({
    name: 'ProductManagement',
    components: { BasicTable, ProductDrawer, TableAction, Button, DeleteOutlined },
    setup() {
      const { t } = useI18n();
      ...
      const [registerTable, { reload }] = useTable({
        title: t('iu.product.productList'),
        api: getProductList,
        columns,
        formConfig: {
          labelWidth: 80,
      ...
      async function handleDelete(record: Recordable) {
        const result = await deleteProduct({ ids: [record.id] });
        if (result.code === 0) {
          await reload();
        }
      }
      ...
      async function handleBatchDelete() {
        Modal.confirm({
          ...
          async onOk() {
            const result = await deleteProduct({ ids: selectedIds.value as number[] });
            if (result.code === 0) {
