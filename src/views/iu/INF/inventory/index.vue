...
<script lang="ts">
  ...
  import { getInventoryList, deleteInventory } from '/@/api/iu/inventory';

  export default defineComponent({
    name: 'InventoryManagement',
    components: { BasicTable, InventoryDrawer, TableAction, Button, DeleteOutlined },
    setup() {
      const { t } = useI18n();
      ...
      const [registerTable, { reload }] = useTable({
        title: t('iu.inventory.inventoryList'),
      ...
      async function handleDelete(record: Recordable) {
        const result = await deleteInventory({ ids: [record.id] });
        if (result.code === 0) {
          await reload();
        }
      }
      ...
      async function handleBatchDelete() {
        Modal.confirm({
          ...
          async onOk() {
            const result = await deleteInventory({ ids: selectedIds.value as number[] });
            if (result.code === 0) {
